...
export default function App({onLock,hasPassphrase}) {
  ...
  if(!state)return <div className="loading-screen"><span className="wordmark">Now<span>.</span></span><p>{error || 'Opening your desk…'}</p>{error&&<div className="button-row"><button onClick={retry}>Retry</button><button onClick={download}>Download old backup</button></div>}{importError&&<p>{importError}</p>}</div>;
  if(hidden)return <div className="curtain"><span className="wordmark">Now<span>.</span></span><p>A little space.</p><button onClick={()=>setHidden(false)}>Return to your desk<Icon name="arrow"/></button><small>Screen hidden. This is not a security lock.</small></div>;
  ...
  return <div className="app-shell" aria-busy={busy}>
    ...
    <main>
      {view==='now'?<>
        <div className="workspace-heading"><div><span className="eyebrow">Your attention, right here</span><h1>One thing at a time.</h1></div><span className="today">{new Date(clock).toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric'})}</span></div>
