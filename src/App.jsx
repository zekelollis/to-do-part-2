...
export default function App({onLock,hasPassphrase}) {
  ...
  const retrySave=async()=>{const pending=pendingAction;const ok=await retry();if(ok&&pending?.type==='add'&&draft===pending.title)setDraft('');if(ok&&pending?.type==='edit'&&editor?.task.id===pending.id)setEditor(null);};
  ...
  return <div className="app-shell" aria-busy={busy}>
    ...
    {editor&&<TaskEditor task={editor.task} state={state} act={act} onClose={()=>setEditor(null)} initialFocus={editor.focus} retrySave={retrySave} pendingSave={!!pendingAction}/>}
