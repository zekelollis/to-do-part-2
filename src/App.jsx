...
export default function App({onLock,hasPassphrase}) {
  ...
  const row=(t,compact=false)=><div className={`task-row ${compact?'compact':''}`} key={t.id}>
    ...
    {t.done||t.deletedAt?<button className="text-button" onClick={()=>act({type:'restore',id:t.id})} disabled={busy}>Restore</button>:compact?<div className="periphery-actions" role="group" aria-label={`Quick actions for ${t.title}`}><button className={`icon-button ${state.flagged.includes(t.id)?'is-flagged':''}`} title={state.flagged.includes(t.id)?'Remove flag':'Flag card'} aria-label={`Flag ${t.title}`} aria-pressed={state.flagged.includes(t.id)} onClick={()=>act({type:'flag',id:t.id})} disabled={busy}><Icon name="flag" size={17}/></button><button className="icon-button" title="Put on deck" aria-label={`Put ${t.title} on deck`} onClick={()=>act({type:'deck',id:t.id})} disabled={busy}><Icon name="deck" size={18}/></button><button className="icon-button row-up" title="Make this now" aria-label={`Make ${t.title} now`} onClick={()=>act({type:'now',id:t.id})} disabled={busy}><Icon name="arrow"/></button></div>:<button className="icon-button" aria-label={`Actions for ${t.title}`} onClick={()=>openTask(t)}><Icon name="more"/></button>}
  </div>;
